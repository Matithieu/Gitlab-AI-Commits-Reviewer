...
import { Config, GitLabMergeRequest, MRSummary } from './types';
...
function summarizeMRs(mrs: GitLabMergeRequest[]): MRSummary {
  ...
  for (const mr of mrs) {
    const projectName = mr.project_name || `Project ${mr.project_id}`;
...
function formatForLetter(summary: MRSummary): string {
  ...
  lines.push('');
  lines.push(`**Total Merged MRs:** ${summary.totalMRs}`);
  lines.push(`**Projects Contributed:** ${summary.projectsContributed.length}`);
  lines.push(`**Period:** ${summary.dateRange.from} to ${summary.dateRange.to}`);
  lines.push('');

  lines.push('## Projects & Contributions');
  lines.push('');
  const sortedProjects = Object.entries(summary.mrsByProject)
    .sort(([, a], [, b]) => b - a);

  for (const [project, count] of sortedProjects) {
    lines.push(`- **${project}**: ${count} merged MRs`);
  }
  lines.push('');
  ...
  lines.push('');
  lines.push('Based on the merge requests listed above, please provide 3-5 bullet points summarizing the key contributions and achievements.');
  lines.push('');
...
async function main() {
  ...
  try {
    ...
    if (projectPaths && projectPaths.length > 0) {
      console.info(`\nUsing config.json with ${projectPaths.length} specified projects`);
    } else {
    ...
    console.info(`\nFound ${mrs.length} merged MRs total.`);
