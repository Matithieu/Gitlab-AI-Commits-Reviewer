import { type GitLabMergeRequest, type GitLabProject } from './types'
...
export class GitLabClient {
  ...
  async getUserProjects(): Promise<GitLabProject[]> {
    return await this.fetchAllPages<GitLabProject>('/projects', {
      membership: 'true',
      simple: 'false',
    })
  }
  ...
  async getProjectByPath(pathWithNamespace: string): Promise<GitLabProject> {
    ...
    return await this.fetchApi<GitLabProject>(`/projects/${encodedPath}`)
  }
  ...
  async getProjectsByPaths(paths: string[]): Promise<GitLabProject[]> {
    ...
    for (const path of paths) {
      try {
        ...
        console.warn(`Could not fetch project "${path}": ${String(error)}`)
      }
    }
    return projects
  }

  async getProjectMergedMRs(
    projectId: number,
    ...
    try {
      const mrs = await this.fetchAllPages<GitLabMergeRequest>(
        `/projects/${String(projectId)}/merge_requests`,
        params,
      )
      return mrs.map((mr) => ({ ...mr, project_id: projectId }))
    } catch (error) {
      console.warn(`Could not fetch MRs for project ${String(projectId)}: ${String(error)}`)
      return []
    }
  }

  async getAllUserMergedMRs(
    username: string,
    projectPaths?: string[],
  ): Promise<GitLabMergeRequest[]> {
